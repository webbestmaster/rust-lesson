...
fn post_gcd(request: &mut Request) -> IronResult<Response> {
    ...
    let unparsed_numbers = match form_data.get("n") {
        None => {
            response.set_mut(status::BadRequest);
            response.set_mut(format!("Form data has not 'n' parameter"));
            return Ok(response);
        }
        Some(nums) => nums
    };
    ...
    writeln!(std::io::stdout(), "unparsed_numbers {:?}", unparsed_numbers).unwrap();
    ...
    writeln!(std::io::stdout(), "numbers {:?}", numbers).unwrap();
